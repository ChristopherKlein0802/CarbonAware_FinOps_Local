...
def _render_instance_detail_table(dashboard_data: Any) -> None:
    ...
    if table_data:
        ...
        st.caption(f"🎯 **Current Grid Intensity**: {grid_intensity:.0f} g CO₂/kWh (German electricity grid)" if grid_intensity else "⚠️ Grid intensity data unavailable")
        st.caption("📊 **Runtime**: Monthly usage hours (CloudTrail-enhanced precision when available)")

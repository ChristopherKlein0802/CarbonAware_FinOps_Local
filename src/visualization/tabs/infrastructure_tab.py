...
class InfrastructureTab:
    ...
    def create_layout(self) -> html.Div:
        ...
        return html.Div([
            ...
            html.Div([
                ...
                # Row 2: Optimization Actions + Cost Opportunities
                html.Div([
                    html.Div([
                        html.H4("💡 Optimization Actions", style={'color': '#333', 'marginBottom': '10px'}),
                        html.Div(id='optimization-actions')
                    ], style={'width': '48%', 'display': 'inline-block', 'marginRight': '4%'}),
                    
                    html.Div([
                        html.H4("🔍 Cost Optimization Opportunities", style={'color': '#333', 'marginBottom': '10px'}),
                        html.Div(id='cost-optimization-opportunities')
                    ], style={'width': '48%', 'display': 'inline-block'})
                ], style={'marginBottom': '30px'})
            ], style={'marginBottom': '40px'}),
    ...
    def create_optimization_actions(self, data: List[Dict]) -> html.Div:
        """Create optimization actions recommendations"""
        if not data:
            return html.Div("No data available for optimization analysis")
        ...
        underutilized = len([item for item in data if item['runtime_hours_month'] / 720 < 0.3])
        high_cost = len([item for item in data if (item['monthly_cost_eur'] / item['runtime_hours_month'] if item['runtime_hours_month'] > 0 else 0) > 0.15])
        
        if underutilized > 0:
            savings = sum(item['monthly_cost_eur'] * 0.4 for item in data if item['runtime_hours_month'] / 720 < 0.3)
            actions.append({
                'action': '⏰ Implement Scheduling',
                'description': f'Schedule {underutilized} underutilized instances during business hours only',
                'impact': f'Save €{savings:.0f}/month',
                'effort': 'Low',
                'timeframe': '1-2 weeks'
            })
        ...
        if high_cost > 0:
            actions.append({
                ...
                'description': f'Review and optimize {high_cost} high-cost instances',
                'impact': f'Save 15-25% on these instances',
                'effort': 'Medium',
                'timeframe': '2-4 weeks'
            })
        ...
        if total_cost > 100:
            actions.append({
                ...
                'impact': 'Save 30-60% on committed usage',
                'effort': 'Low',
                'timeframe': '1 week'
            })
        ...
        if not actions:
            return html.Div([
                html.P("✅ Infrastructure is well-optimized", style={'color': '#2E8B57', 'fontSize': '16px'})
            ])
        
        return html.Div([
            html.Div([
                html.Div([
                    html.H5(action['action'], style={'color': '#2E8B57', 'margin': '0 0 10px 0'}),
                    html.P(action['description'], style={'margin': '5px 0', 'fontSize': '14px'}),
                    html.Div([
                        html.Span(f"💰 {action['impact']}", style={'color': '#666', 'fontSize': '12px', 'marginRight': '15px'}),
                        html.Span(f"⚡ {action['effort']} effort", style={'color': '#666', 'fontSize': '12px', 'marginRight': '15px'}),
                        html.Span(f"⏱️ {action['timeframe']}", style={'color': '#666', 'fontSize': '12px'})
                    ])
                ], style={
                    'backgroundColor': 'white',
                    'borderRadius': '8px',
